                ...
                <div className="content">
                  {
                    categoriesTree.map(({Category}, i) => {
                      ...
                      return <div key={i} className={`category-l1-menu ${(selectedCategory.ID === ID ? "selected" : "")}`}
                                ...
                                <Link key={i} to={urlGenerator.get({page: 'category', id: FriendlyID})}><a key={i} className="category-name truncate">{Name}</a></Link>
                            </div>
                    })
                  }
                </div>
        ...
        <div className="category-panel">
          <div className="category-l1-title">
            <span className="category-l1">
              <Link to={urlGenerator.get({page:'category', id:selectedCategory.FriendlyID})}><a>{selectedCategory.Name}</a></Link>
            </span>
            {
              selectedCategory.HasProducts &&
              <span className="featured-products">
                <Link to={urlGenerator.get({page:'category', id:selectedCategory.FriendlyID})}>
              ...
              subCategoriesMapped && subCategoriesMapped.map((categoryL2, i) => {
                ...
                return <React.Fragment key={i}>
                  ...
                  <div key={i} className="column category">
                    <Link key={i} to={urlGenerator.get({page: 'category', id: FriendlyID})}><a key={i} className="category-l2 truncate">{Name}</a></Link>
                  </div>
                  {
                    showFeaturedProducts &&
                    <div key="featured-products" className="column category">
                      <Link key="key-featured-products" to={urlGenerator.get({page: 'category', id: FriendlyID})}><a key="key-featured-products" className="category-l3 truncate">
                  ...
                  }
                  {
                    subCategories && subCategories.map((categoryL3, i) => {
                      ...
                      return <div key={i} className="column category">
                        <Link key={i} to={urlGenerator.get({page: 'category', id: FriendlyID})}><a key={i} className="category-l3 truncate">{Name}</a></Link>
                      </div>
                    })
                  }
                </React.Fragment>
          ...
          </div>
          {
            ...
            (countAll > countInColumn * NUMBER_OF_COLUMNS)?
              <div className="see-more">
                <Link to={urlGenerator.get({page:'category', id:selectedCategory.FriendlyID})}><a className="see-more-link">{t('NavigationMenu.See_More')}</a></Link>
              </div>: null
          }
        </div>
      </div>
