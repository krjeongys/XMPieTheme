...
import { sleep, formatDateByLocale } from '$ustoreinternal/services/utils'
...
class KitProduct extends Component {
  ...
  getData = async function (productID, orderItemID) {
    ...
    if (orderItemID) {
      orderProduct = await UStoreProvider.api.orders.getOrderItem(orderItemID.toUpperCase())
    } else {
  ...
  onClickContinue() {
    ...
    if (this.state.showLoadingOnContinue) return

    const { customState: { currentOrderItem, kitViewModel: { items } } } = this.props

    const hasIncluded = currentOrderItem.Items.find(item => item.IsIncluded) !== undefined
  ...
  async onReorder() {
    ...
    const newURL = `${urlGenerator.get({ page: 'products', id: currentProduct.FriendlyID })}?OrderItemId=${newOrder.ID}&reorder=true`
  ...
  render() {
    ...
    const hasIncluded = !currentOrderItem ? null : currentOrderItem.Items.find(item => item.IsIncluded) !== undefined
