...
class KitItem extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ isMediumBreak: document.body.clientWidth <= parseInt(theme.lg.replace('px', '')) })

    const input = document.getElementsByClassName('.quantity-control.quantity-input')
    if (input && input.length === 1) input[0].addEventListener('focus', function (e) { e.target.scrollIntoView(true, { behavior: 'smooth' }) })
  ...
  onValidateIframeReturn(isPropertiesValid) {
    ...
    if (!this.externalValidation && isPropertiesValid) {
      this.iframeElement && this.iframeElement.contentWindow.location.replace(this.getIframeSrc(ID))
  ...
  getIframeSrc(orderItemID) {
    ...
    return `${classicUrl}/ProductProperties.aspx?OrderItemId=${orderItemID}&${viewMobileVal}&rand=${Math.random()}`
  }
  ...
  renderEditLink(className, isOutOfStock) {
    ...
    return (
      <div className={'edit-section ' + className}>
        ...
        {kitItemOrderItemModel.IsIncluded && !kitItemViewModel.iframeLoading && !isOutOfStock && (<span className={'edit-options'} onClick={this.onClickEditOptions}><ResponsiveEllipsis text={t('KitItem.Edit_options')} maxLine={2} basedOn='words' /></span>)}
        {(!kitItemOrderItemModel.IsIncluded || isOutOfStock) && (<span className='disabled-edit-options'><ResponsiveEllipsis text={t('KitItem.Edit_options')} maxLine={2} basedOn='words' /></span>)}
      </div>
    )
  }
          ...
          <div className='item-name-price-quantity'>
            <div className='item-name-and-price'>
              <div className='item-name'>
                <ResponsiveEllipsis text={itemName} maxLine={2} basedOn='words' />
        ...
        <Modal isOpen={this.state.isPreviewOpen} modalClassName="kit-preview-modal" toggle={this.togglePreviewModal}>
          <ModalBody>
            <div>
              ...
              <div className='modal-title'><ResponsiveEllipsis maxLine={1} text={Name} /></div>
